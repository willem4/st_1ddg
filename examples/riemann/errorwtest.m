...
% for height
    e(2,1:Nn)=a(2,1:Nn);%+a(2,2*Nn+1:3*Nn);
    f(1,1:Nn)=b(1,1:Nn);
% for speed
...
%    c(2,:) = abs(c);
    d(2,:) = c(2,:).^2;
    g1 = c(2,length(c)*0.7+1:length(c)*0.8);
    g2 = c(2,length(c)*0.8+1:length(c)*0.9);
    g1 = 0.5*(g1 + abs(g1));
    g2 = 0.5*(-g2 + abs(g2));
    nrminf = norm([g1 g2],inf);    
    %nrminf = norm(c(2,length(c)*0.7+1:length(c)*0.9),inf);
    [norm(g1) norm(g2)]
    
    nrm1 = 0;
    nrm2 = 0;
    for ii=length(c)*0.7+1:2:length(c)*0.9,
